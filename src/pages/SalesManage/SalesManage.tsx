...
import {
  ...
  SALES_KEY, VAR_KEY, FIX_KEY, EMPLOYEE_AUTO_KEY, parse,
} from "./salesData";
...
export default function SalesManage() {
  ...
  const [variableEntries, setVariableEntries] = useState<VariableExpenseEntry[]>([]);
  ...
  useEffect(() => {
    ...
    setVariableEntries(parse<VariableExpenseEntry[]>(localStorage.getItem(VAR_KEY), []));
    ...
    <S.Page>
      <S.Layout>
        <S.Sidebar>
          ...
          <S.MenuButton type="button" $active={activeTab === "check"} onClick={() => setActiveTab("check")}>
            매출 확인
          </S.MenuButton>
          <S.MenuButton type="button" $active={activeTab === "input"} onClick={() => setActiveTab("input")}>
            매출 입력
          </S.MenuButton>
          <S.MenuButton type="button" $active={activeTab === "report"} onClick={() => setActiveTab("report")}>
            보고서 확인
          </S.MenuButton>
        ...
        <S.Content>
          {activeTab === "check" && (
            <SalesCheck salesEntries={salesEntries} variableEntries={variableEntries} fixedMap={fixedMap} />
          )}
          {activeTab === "input" && (
            <SalesInput
